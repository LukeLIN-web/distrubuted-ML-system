...
我不知道为啥sl 还要 val_jmr? 算一个loss不就行了吗? 为啥要知道reward? 好像是因为要训练一会儿,  然后validation做一下DRL决策看看对不对. val_jmr 需要rl_step, 我不明白 env.step(output)的 masked_output = np.reshape(output[0] * mask, (1, len(mask)))  应该怎么reshape.
