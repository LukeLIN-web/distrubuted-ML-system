...
我们提出, 不用人工分组的方法. 用two-level hierarchical network. 第一个模型把graph的操作分组(问题 :  这个graph的op 是哪里来的? 是tf自动生成的吗? 我觉得是的), 第二个模型把分组放在设备上. group一个feed forward NN 前馈网络, graph中读取每个操作和上下文信息, 预测应该分配哪个组. Placer 是一个序列到序列模型, 读取 the embedding of the group(不知道embedding啥意思) 并预测该group的placement.  用RL 对两级网络联合训练. (老师想做的是一级resource allocation, 一级placement. )我们的方法是端到端的. 不需要人工分group. 
